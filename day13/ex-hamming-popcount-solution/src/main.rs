...
///
/// XOR gives us a `u64` whose 2-bit slots are non-zero exactly at mismatched positions.
/// For the encoding A=00, C=01, G=10, T=11 a mismatched pair can differ in 1 or 2 bits,
/// so we cannot just popcount and divide. Instead we fold each 2-bit slot to a single
/// "any-mismatch" bit by ORing the two halves together, then mask to one bit per slot
/// (0x5555... is 0101...0101 in binary) and count.
