...
/// Return the Watson-Crick complement of a single DNA base.
///
/// Preserves case for A, T, C, G. Passes `N` and `n` through unchanged.
/// Any other byte should panic with a clear message.
fn complement_base(base: u8) -> u8 {
    // TODO:
    //   Match on `base` and return the complement.
    //     A <-> T, C <-> G, in both upper and lower case.
    //     N and n pass through unchanged.
    //     For any other byte, call panic! with a message that includes
    //     the offending byte.
    let _ = base;
    0
}
