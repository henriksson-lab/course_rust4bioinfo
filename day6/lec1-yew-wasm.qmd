...
**REST** [Representational State Transfer — the convention of using HTTP verbs (GET, POST, ...) plus URL paths to access named resources]. Before adding a frontend, here's the smallest possible backend — it accepts a DNA sequence and returns its GC content. `axum` [a Rust web framework for building HTTP servers] is the backend library we use today:
...
A **controlled input** [a form element whose displayed value is read from state, and whose changes are written back to state] makes the Rust value the single source of truth.
...
[`axum`](https://docs.rs/axum/) is the standard Rust web framework for JSON APIs. A **route** [a mapping from URL path + HTTP method to a handler] is added with `Router::new().route(...)`. A **handler** [the async function axum calls when a route matches] is just an `async fn` that returns something serialisable.
...
An **origin** [the (scheme, host, port) triple — `http://localhost:8080` and `http://localhost:3000` differ in port, so they are different origins] is what the browser uses to decide who can talk to whom. **CORS** [Cross-Origin Resource Sharing — HTTP headers the server sends to opt in to cross-origin requests] is the permission system on top of that.

Recall ports — your frontend on `:8080` calling your backend on `:3000` is technically two different origins. The browser's **Same-Origin Policy** [the default rule that scripts on page A cannot read responses from a different origin B] blocks the call unless the server explicitly opts in.
