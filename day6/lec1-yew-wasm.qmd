...
These three words come up constantly. They name the parts of every web app you've ever used. Today we go full-stack, in Rust, end to end.
...
::: notes
For us today the split is literal — two crates, two processes, two ports.
:::
...
When the browser parses your HTML, it builds an in-memory tree of nodes called the **DOM** (Document Object Model). Every box in the rendered page is a node in this tree. Everything that changes the page after load — JavaScript, jQuery, React, Yew — does so by **manipulating the DOM tree**.
...
::: notes
This is the foundation for understanding what Yew actually does (next part of the lecture). Yew = "build me a virtual DOM in Rust, then diff against the real one and patch the differences". The HTML on the right is the same example the tree on the left depicts — each tag becomes a node.
:::
...
## A REST server in 20 lines of axum {.smaller}
...
## The build pipeline — step 1: Rust source
...
::: notes
Stage one: the Rust crate as you write it. Plain `main.rs`, `lib.rs`, `Cargo.toml` — no wasm-specific code yet.
:::

## The build pipeline — step 2: cargo + wasm-bindgen
...
::: notes
Stage two: the Rust compiler emits a `.wasm` binary, and `wasm-bindgen` writes a small JavaScript shim that knows how to call into the wasm exports.
:::

## The build pipeline — step 3: trunk bundles
...
::: notes
Stage three: trunk pulls the `.wasm`, the JS glue, `index.html`, and any assets into a `dist/` directory and serves them over HTTP. The wasm runs in its own virtual machine inside the same browser process as the JavaScript VM — they share memory only through carefully typed bridges.
:::
...
## `use_state` — a value that survives re-renders {.smaller}
...
## Controlled input — input bound to state {.smaller}
