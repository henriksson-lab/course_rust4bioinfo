...
The `1` here is the small label (the **tag** [a byte that records which variant this value is — Leaf or Internal]) Rust stores so it can tell which variant a `Node` value is. The `2 × size(Node)` is the two child Nodes inlined into the parent — that's the part that has no finite answer.
...
The explicit shape first — an accumulator and a loop, exactly as you'd write in Python:
...
The first version is the one most readers can read off without thinking: declare a mutable accumulator, loop, add, return. The second uses iterators: `map(count_tips)` produces an iterator of usize values, and `sum()` reduces them. `count_tips` is passed by name as a function value to `map` — Rust functions are first-class.
...
Every recursive call pushes a **stack frame** [a chunk of memory holding the call's local variables and return address] onto the **call stack** [the program's per-thread stack of in-progress function calls].
...
Rust does not guarantee **tail-call optimisation** [a compiler trick that reuses a single stack frame for recursive calls in tail position — so it can't overflow]; some other languages do, but you can't rely on it here.
