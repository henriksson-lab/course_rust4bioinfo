...
The obvious recursion follows the English: at every step, either align the last two characters, drop one from `a`, or drop one from `b`. Take the best of the three.
...
Each call makes three more calls. To compute `align("CA", "CA")` we end up computing `align("C", "C")` three different ways — and each of those branches three more times. The same little subproblem ["a subproblem is the same kind of question on smaller inputs"] is recomputed over and over.

That's where the explosion comes from. The next slide quantifies it.
...
*This rule (CS jargon: the **recurrence** [the formula that defines each cell in terms of smaller ones]) is the heart of the algorithm. We'll call it the "filling step" or "the rule" from here on.*
...
**Trick:** at each cell, also remember **which neighbour won** the max. Walking those back-pointers from the bottom-right cell to the top-left reproduces the alignment — this walk is called the **traceback** [reading the choices back out of the filled table to recover the actual alignment, not just its score].

![Same matrix, now read backwards: trace the arrows from the bottom-right cell to the top-left.](images/dp-matrix.svg){fig-alt="The same Needleman-Wunsch matrix as on the previous slide, reused here so the audience can mentally trace the back-pointers from (n, m) back to (0, 0)."}

The path through the matrix for the worked `CACGT` vs `CAGT` example, with each step labelled by which neighbour won:
...
Reading the chosen arrows along the path bottom-right → top-left gives:
