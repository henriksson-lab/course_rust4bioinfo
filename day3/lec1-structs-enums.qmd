...
- **The Rust types that you spend the most time with.**
...
## `impl` — methods belong to the type {.smaller}
...
```r
GenomicInterval <- R6::R6Class(
  "GenomicInterval",
  public = list(
    chrom = NULL,
    start = NULL,
    end   = NULL,
    initialize = function(c, s, e) {
      self$chrom <- c
      self$start <- s
      self$end   <- e
    },
...
Call site: `iv.length()` (Rust), `iv.length()` (Python), `iv$length()` (R).
...
## More methods — mutation and construction {.smaller}
...
Same idea — `__init__` is the constructor, ordinary methods mutate via `self`. The big difference: Python won't stop you mutating from anywhere; Rust's `&mut self` is checked at compile time.
:::
::::
...
## `enum` — one of a fixed set of named values {.smaller}
...
**Memory cost of one strand value:**

- **Python** `strand = "+"` — typically ~50 bytes (object header + interning + character storage)
- **Rust** `let strand = Strand::Plus;` — **1 byte** (enum discriminant)

A 10 million-row table of strands takes roughly ~500 MB in Python vs ~10 MB in Rust.
...
Control flow: a value enters from the top; arms are tried top-to-bottom; the first matching arm's body runs; the result is the result of the match expression.
:::

## Exhaustiveness — the safety net {.smaller}
...
```text
error[E0004]: non-exhaustive patterns: `Minus` not covered
 --> src/main.rs:8:11
  |
8 |     match strand {
  |           ^^^^^^ pattern `Minus` not covered
  |
  = note: ensure that all possible cases are being handled by adding
...
## match ensures you do not forget to add needed logic

![A missing case is a build error, not a runtime surprise.](images/lec1-clinical-exhaustiveness.svg){fig-alt="Side-by-side: left, Python if/elif chain missing one case, with a red arrow showing a silently dropped sample; right, Rust match with a compile-error diagnostic pointing at the missing variant."}
...
::: {.column width="45%"}
![Result has two output branches.](images/lec1-result-flow.svg){fig-alt="A function returning Result<T, E> shown with two output branches; the Ok(value) branch flows into the caller's next statement, the Err(e) branch routes to an error handler or short-circuits out."}
:::
::::
...
## The ? operator is a nice shorthand
...
Four lines per fallible call, every time.
:::
::::
...
## `?` in a real parser — `ok_or` and `map_err` {.smaller}
...
On the right: this is what `?` desugars to. The call returns a Result. On `Ok(value)`, control falls through to the next line of the function and `value` is bound. On `Err(e)`, the same `e` is returned from the enclosing function — execution skips everything below the `?`. Each `?` in your code is a possible early-return path.
:::
...
The picture on the right shows the mental model: elements flow left to right through the chain. The filter stage examines each one and either lets it pass or drops it. The `count` consumer is on the receiving end, pulling elements until the source is exhausted, incrementing a counter each time. The chain doesn't materialise any intermediate Vec — each byte is checked and either counted or discarded in place.
:::
...
if "ATG" in counts:
    n = counts["ATG"]
    print(f"seen ATG {n} times")
```

Same data structure. Rust forces you to spell out the key and value types up front; Python infers everything at runtime.
:::
::::
...
## The entry API — read-modify-write in one call {.smaller}
...
The figure shows 7 windows in ATGATGCAT; ATG appears twice. Each window does one entry-API lookup; new keys are inserted with value 0 then incremented; existing keys are incremented in place. Total: 7 hash lookups, 6 allocations for new keys.
:::

## Closures can replace small functions
...
let high_quality = quals.iter()
    ...
    // captures `threshold` from outside
    .count();
```
