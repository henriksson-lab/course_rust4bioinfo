...
Same idea — fields, methods, a `new`-equivalent — different syntax.
:::
:::
...
A function in an `impl` block with no `self` is an "associated function" — a constructor or helper attached to the type. The `Self` keyword inside the block means "the type we're implementing".

The Python side shows the equivalent class. `__init__` plays the role of our `new` associated function; `self` in Python is the explicit first parameter, same as Rust. The big practical difference: Rust distinguishes read-only borrows (`&self`) from mutable borrows (`&mut self`) at the type level, where Python silently allows mutation everywhere.
...
The compiler writes the obvious trait implementations for you — equality, debug printing, cloning — so you don't have to type them by hand every time.
...
::: notes
The simple enum (`Strand`) compiles down to a single small integer tag. A sum-type enum is a tag plus a union of all variant payloads — every value reserves room for the largest variant. So even an `Snp` carries a slot the size of the `Insertion` payload, most of it unused.
