...
The compiler writes the obvious **trait** [a named bundle of methods a type can opt in to — like a Java interface, but more powerful] implementations for you — equality, debug printing, cloning — so you don't have to type them by hand every time.
...
- **`Copy`** — assignment copies (only for stack-only types)
  - only works if every field is also `Copy` — so not for anything holding a `String` or `Vec`
- **`PartialEq`** / `Eq` — `==` and `!=`
...
# or: "het " (trailing space) silently
# ignored
```
...
![Three ways to pattern-match an Option<T>.](images/lec1-option-flow.svg){fig-alt="A value of type Option<T> enters; the Some(x) arm uses x; the None arm runs a fallback. Three pattern styles shown side by side: match, if let Some(x), and unwrap_or(default)."}
...
Idiomatic one-liner once you're used to it: `*counts.entry(window.to_vec()).or_insert(0) += 1;`
...
The `*q` dereferences once: `quals.iter()` yields `&u8` (a reference to a byte), and `filter` passes a reference to *that* — so inside the closure `q` is a `&&u8` and `*q` reads the byte. You'll sometimes see the shorter `|&&q| q >= threshold` form that destructures both layers in the pattern; same meaning.
...
The reference-layering is one of the small surprises Rust hands you in iterator chains. `iter()` yields `&T` so it doesn't copy or move elements; `filter` then borrows each element to look at it, which gives you a `&&T` inside the closure body. You either write `|q| *q >= threshold` (deref in the body) or `|&&q| q >= threshold` (destructure both layers in the pattern) — same compiled code.
