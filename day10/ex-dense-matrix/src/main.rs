...
fn trace(a: &Array2<f64>) -> f64 {
    // TODO: iterate over min(rows, cols) and sum a[[i, i]]
    let _ = a;
    0.0
}
...
fn transpose(a: &Array2<f64>) -> Array2<f64> {
    // TODO: ndarray has a built-in `.t()` method that returns a view;
    // call `.t()` and convert to an owned Array2 with `.to_owned()`.
    let _ = a;
    Array2::zeros((0, 0))
}
...
fn matvec(a: &Array2<f64>, v: &Array1<f64>) -> Array1<f64> {
    // TODO: ndarray's Array2 has a `.dot(...)` method that does matrix-vector
    // and matrix-matrix multiplication; call a.dot(v).
    let _ = (a, v);
    Array1::zeros(0)
}
