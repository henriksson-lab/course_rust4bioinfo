...
fn step(p: &Array1<f64>, transition: &Array2<f64>) -> Array1<f64> {
    // TODO: matrix-vector product where p is a row vector.
    // Easiest path: transition.t().dot(p) — multiply the transpose of P
    // by p as a column vector. Equivalent to "p · P" for the row convention.
    let _ = (p, transition);
    Array1::zeros(0)
}
...
fn power_iterate(p_0: Array1<f64>, transition: &Array2<f64>, n: usize) -> Array1<f64> {
    // TODO: loop n times, replacing p with step(&p, transition) each iteration.
    let _ = (transition, n);
    p_0
}
