...
::: {.fragment}
![Two team members type the same strain's name slightly differently — "K-12 MG1655" vs "K12 MG1655". Now your query for one returns half the data.](images/lec1-pain-step1.svg){fig-alt="Two team members typing the same strain's name slightly differently, leaving the TSV with inconsistent entries."}
:::

::: {.fragment}
![A row gets pasted twice. The same strain now has two `strain_id`s, two contradictory `isolation_source` values, and there's no way to tell which one is right.](images/lec1-pain-step2.svg){fig-alt="A duplicated row showing the same strain with two strain_id values and contradictory isolation_source entries."}
:::

::: {.fragment}
![Two people open the TSV in Excel at the same time. One saves, the other saves, the second save silently overwrites the first.](images/lec1-pain-step3.svg){fig-alt="Two users opening the same TSV in Excel, with the second save overwriting the first."}
:::

A relational database stops every one of these at the moment they happen — not after the paper is published.

::: notes
This is the universal story: a shared TSV grows past the point where ad-hoc edits can be trusted. A relational database is the standard, fifty-year-old answer to every one of these failure modes. Today is the introduction.
:::
...
![Each row's `strain_id` is unique — that's what makes it a primary key.](images/lec1-primary-key-example.svg){fig-alt="A strain table where every row has a distinct strain_id value, highlighted as the primary key."}
...
If every assay record had to repeat the full strain name, species, and isolation source, two things go wrong:

1. Storage explodes (the same strain has hundreds of assays).
2. Updates are unsafe — change a strain's species in one row, forget to change it in the other 99, and the data set quietly disagrees with itself.

The relational solution: store each strain once. Every assay carries just the strain's `strain_id` and "joins" against the strain table when needed. This is called **normalisation** (storing each fact in exactly one place).

![Without references, the same strain appears redundantly in every assay row, and an update misses some copies — the two copies quietly disagree.](images/lec1-why-tables-reference.svg){fig-alt="A wide assay table where strain metadata is repeated on every row, with one row's species edited and the others left stale."}
...
![Before INSERT, the table has 3 rows. After, the new row is appended; the `strain_id` is auto-assigned.](images/lec1-insert-row.svg){fig-alt="Before/after view of the strain table: three rows, then four after the INSERT, with the new row's strain_id auto-assigned."}
...
The result is a table of just the columns you asked for:

![Output of `SELECT strain_name, species FROM strain WHERE year_isolated > 1980;` — three columns become two, four matching rows.](images/lec1-select-output.svg){fig-alt="Output table of the SELECT query, showing only strain_name and species columns for rows where year_isolated exceeds 1980."}
...
**Aggregation** (collapsing many rows down to one summary row per group) is the SQL idiom for "how many", "what's the average", "what's the max".
...
![Input: the `assay` table (one row per measurement). Output: one row per medium, with `AVG(od600_24h)` computed across all rows in that group.](images/lec1-aggregation-example.svg){fig-alt="An input assay table with many rows mapped to an output table with one row per medium, showing the AVG(od600_24h) computed per group."}
...
The clauses you write in a `SELECT` don't run in writing order — they run as a pipeline:

::: {.fragment}
![Stage 1 — `FROM table`: pick the input table(s).](images/lec1-select-pipeline-step1.svg){fig-alt="Pipeline stage 1: the FROM clause selects the input table(s)."}
:::

::: {.fragment}
![Stage 2 — `WHERE`: filter rows.](images/lec1-select-pipeline-step2.svg){fig-alt="Pipeline stage 2: the WHERE clause filters individual rows."}
:::

::: {.fragment}
![Stage 3 — `GROUP BY`: bucket rows by a key.](images/lec1-select-pipeline-step3.svg){fig-alt="Pipeline stage 3: the GROUP BY clause buckets rows by a key."}
:::

::: {.fragment}
![Stage 4 — `SELECT` and aggregations: compute columns per group.](images/lec1-select-pipeline-step4.svg){fig-alt="Pipeline stage 4: the SELECT clause and aggregations compute columns per group."}
:::

::: {.fragment}
![Stage 5 — `ORDER BY` and `LIMIT`: sort and trim.](images/lec1-select-pipeline-step5.svg){fig-alt="Pipeline stage 5: the ORDER BY and LIMIT clauses sort and trim the result."}
:::

This is why `WHERE` can reference table columns but `HAVING` references aggregations — they run at different stages.

::: notes
You write the clauses in declarative order — SELECT first, FROM second — but the engine runs them in execution order: pick tables, drop rows, bucket, aggregate, project columns, sort, cut. Understanding this order is what makes the difference between WHERE and HAVING click: WHERE filters individual rows before grouping, HAVING filters whole groups after aggregation. You cannot use `COUNT(*) > 1` in WHERE — at WHERE-time the count does not exist yet.
:::
...
::: {.fragment}
![Start: strain table on the left, assay table on the right; no joined rows yet.](images/lec1-inner-join-step1.svg){fig-alt="The strain and assay tables side by side with no joined output yet."}
:::

::: {.fragment}
![Match #1: assay row with `strain_id = 1` joins to strain row 1; produces one combined row.](images/lec1-inner-join-step2.svg){fig-alt="An assay row with strain_id 1 joined to strain row 1, producing one combined output row."}
:::

::: {.fragment}
![Match #2: a second assay for `strain_id = 1` joins to the same strain row; the strain row is reused (one strain, many assays).](images/lec1-inner-join-step3.svg){fig-alt="A second assay for strain_id 1 joined to the same strain row, illustrating one strain with many assays."}
:::

::: {.fragment}
![All matching pairs: every assay whose `strain_id` exists in the strain table appears in the output exactly once per (strain, assay) pair.](images/lec1-inner-join-step4.svg){fig-alt="The complete inner-join output: every assay matched to its strain, one output row per matching pair."}
:::

::: notes
The animation builds the join one matched pair at a time. The same strain row is reused for every assay that points at it — that is what produces the one-to-many shape. A strain with zero matching assays would be dropped from the output entirely — that's the defining behaviour of INNER JOIN. Hold that thought; it is exactly what LEFT JOIN exists to fix.
:::

## NATURAL JOIN — the shortcut

**`NATURAL JOIN`** (auto-join on every column the two tables happen to share by name) is the shortcut. If both tables have a column with the **same name**, SQL can infer the join:
...
![NATURAL JOIN auto-joins on every column with the same name. Here `strain_id` is the only shared column — so it's the join key.](images/lec1-natural-join-columns.svg){fig-alt="Two table headers side by side with strain_id highlighted as the only shared column name, the implicit NATURAL JOIN key."}
...
**NULL** (the database's "no value here" marker — *not* zero, *not* empty string) follows **three-valued logic** (true / false / unknown). Comparing anything to NULL gives NULL, not true or false.

```sql
-- Tiny table with one missing value:
-- assay_id | strain_id | od600_24h
--    101   |     1     |   0.42
--    102   |     1     |   NULL
--    103   |     2     |   0.61

SELECT * FROM assay WHERE od600_24h = NULL;   -- 0 rows!  (NULL = NULL is NULL, not true)
SELECT * FROM assay WHERE od600_24h IS NULL;  -- 1 row    (the right way to ask)

SELECT COUNT(*)          FROM assay;          -- 3   (counts every row)
SELECT COUNT(od600_24h)  FROM assay;          -- 2   (skips the NULL row)
```
...
**`LEFT JOIN`** (keep every row from the left table, even when nothing on the right matches) fills `NULL` on the right when there is no match.

A strain with zero assays shows up with `n = 0`. With `INNER JOIN` it would silently vanish.

![LEFT JOIN keeps every row from the left table, even when there's no match on the right — the unmatched fields become NULL.](images/lec1-left-join-example.svg){fig-alt="LEFT JOIN output retaining all left-table rows; rows with no match on the right show NULL values in the right-side columns."}
...
::: {.fragment}
![Step 1 — `Connection::open(path)`: connect to the database file (creates one if missing).](images/lec1-rusqlite-step1.svg){fig-alt="Step 1: Connection::open opens (or creates) the .db file and returns a Connection handle."}
:::

::: {.fragment}
![Step 2 — `conn.prepare("SELECT ...")`: parse and compile the SQL into a reusable **prepared statement** (a compiled query the engine can run many times with different parameter values).](images/lec1-rusqlite-step2.svg){fig-alt="Step 2: conn.prepare compiles the SQL string into a reusable Statement object."}
:::

::: {.fragment}
![Step 3 — `stmt.query_map(params, |row| ...)`: bind the parameters, run the query, map each row to a Rust struct.](images/lec1-rusqlite-step3.svg){fig-alt="Step 3: stmt.query_map binds parameters, runs the query, and maps each row to a Rust struct via the closure."}
:::

Step 4 is just iterating the `MappedRows` with a `for` loop.
...
```rust
let mut stmt = conn.prepare(                              // compile the query once
    "SELECT strain_id, strain_name, species
     FROM strain
     WHERE species = ?1",
)?;

let rows = stmt.query_map(["Escherichia coli"], |row| {   // bind ?1, run, map each row
    Ok(Strain {                                           // build a Strain struct per row
        strain_id:   row.get(0)?,                         // column 0 -> strain_id field
        strain_name: row.get(1)?,                         // column 1 -> strain_name field
        species:     row.get(2)?,                         // column 2 -> species field
    })
})?;

for strain in rows {                                      // iterate the results
    println!("{:?}", strain?);                            // unwrap the row's Result, then print
}
```
...
**Parameter binding** (sending the SQL template and the values to the database separately, instead of pasting the values into the SQL string) is the only safe way to inject user data into a query.
...
If `user_input` is `"x'; DROP TABLE strain; --"`, you just deleted the table. This is a **SQL injection** (an attacker smuggles SQL syntax inside what your code treats as a plain value).
...
![](https://imgs.xkcd.com/comics/exploits_of_a_mom.png){width="60%" fig-alt="The XKCD 'Exploits of a Mom' comic strip, showing a school calling a parent about their child named 'Robert'); DROP TABLE Students;--' which deleted all student records due to a SQL injection vulnerability."}

In 2015 the UK ISP **TalkTalk** lost the personal details of ~157,000 customers to a SQL injection attack — total cost ~£77 million, including a record ICO fine. A teenager exploited an unpatched legacy page.

→ [Wikipedia: TalkTalk data breach](https://en.wikipedia.org/wiki/TalkTalk_data_breach)
→ [Wikipedia: SQL injection (long list of public incidents)](https://en.wikipedia.org/wiki/SQL_injection)

Even a small lab-internal web app over your strain database is reachable from the campus network — the same rule applies.

Use parameter binding. Always. There is no scenario in which the saved characters of string concatenation are worth £77 million.

::: notes
Bioinformatics rarely runs public web services, but lab databases sometimes back internal web tools — and the lesson generalises: never paste untrusted strings into SQL. rusqlite's `params!` macro makes the safe path the easy one.
:::
