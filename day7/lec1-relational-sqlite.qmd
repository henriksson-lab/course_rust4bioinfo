...
The relational solution: store each strain once. Every assay carries just the strain's `strain_id` and "joins" against the strain table when needed.

![Without references, the same strain appears redundantly in every assay row, and an update misses some copies — silent data drift.](images/lec1-why-tables-reference.svg){fig-alt="A wide assay table where strain metadata is repeated on every row, with one row's species edited and the others left stale."}
...
::: {.fragment}
![Step 2 — `conn.prepare("SELECT ...")`: parse and compile the SQL into a reusable statement.](images/lec1-rusqlite-step2.svg){fig-alt="Step 2: conn.prepare compiles the SQL string into a reusable Statement object."}
:::
