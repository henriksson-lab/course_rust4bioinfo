//! Day 12 — Exercise 2: SAXPY (c = a*x + y) with a uniform buffer.
//!
//! Your job: complete the two TODOs marked below — (1) create the uniform
//! buffer holding the scalar `a`, and (2) add the bind-group entry for it.
...
pub async fn saxpy(a: f32, x: &[f32], y: &[f32]) -> Vec<f32> {
    ...
    // TODO 1: create the uniform buffer `scalar_buf` holding `Scalar { a, _pad: [0.0; 3] }`.
    // Use `device.create_buffer_init` with usage = UNIFORM | COPY_DST.
    // See hint 1 in 02-saxpy.qmd.
    let scalar_buf: wgpu::Buffer = todo!("create the uniform buffer");
    ...
    let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        layout: &bgl,
        entries: &[
            // TODO 2: add a bind-group entry for the uniform buffer at binding 0.
            // The entries below cover bindings 1, 2, 3.
