...
- Written companion: [day 2 — Concepts](00-concepts.qmd)
- ~30 minutes, then four exercises: reverse complement, buffer reuse, k-mers, quality trim
:::
...
![A Vec<u8> with ptr/len/cap on the stack, pointing to an 8-cell heap allocation holding ACGTA.](../day1/images/lec2-vec-memory.svg){fig-alt="Stack/heap split with a Vec<u8> three-field header on the stack and the bytes on the heap."}
...
No `free()`, no garbage collector. The compiler inserts the cleanup at the closing `}`.
...
![Move semantics: the 24-byte Vec header is copied to the new owner; the heap allocation stays put; the old binding is invalidated.](images/lec1-move-semantics.svg){fig-alt="Stack/heap diagram with v greyed out and crossed in red, w now holding the same ptr/len/cap, and the 4-byte heap allocation unchanged."}
...
![Shared borrows: v owns the heap allocation; r and s are both &v and both point to the same data.](images/lec1-borrow-immutable.svg){fig-alt="v with ptr/len/cap on the stack, two &Vec<u8> references r and s also on the stack, all three pointing at the same 4-cell heap allocation containing ACGT."}
...
![Two-panel diagram: left, &mut v as the sole outstanding borrow compiles fine; right, trying to add a second & borrow while &mut is alive is rejected by the compiler.](images/lec1-borrow-mut-exclusive.svg){fig-alt="Left panel labeled OK: v with one &mut v borrow, with note 'sole writer'. Right panel labeled compile error: v with both &mut v and &v, the &v crossed out and the compiler error printed below."}
...
let first = &v[0];          // shared borrow into v's heap allocation
v.push(b'T');               // may REALLOCATE -> first would dangle!
println!("{}", first);      // (in C/C++, undefined behaviour)
```

Rust rejects this at compile time. `first` is a `&u8` into v's buffer; `v.push` needs `&mut v`; the two cannot coexist.

::: notes
This is the canonical example of why the rule matters.

`first` is a reference to the `b'A'` byte in v's heap buffer. `v.push(b'T')` may or may not trigger a reallocation, depending on capacity — and if it does, the old buffer is freed. `first` would then point at freed memory. Read through it, get garbage. Or worse: get whatever the allocator handed out next.

In C++, this is one of the most common sources of crashes in production code — `std::vector::iterator` invalidation. In Java, it's a `ConcurrentModificationException` at runtime, *if you're lucky*. In Rust, the compiler refuses to build the program. Same bug, three different consequences. Rust's is the only one that catches it before shipping.

The fix is mechanical: don't keep `first` alive across the push, or use an index instead of a reference. The compiler tells you exactly which line is the problem.
:::

## Borrowing in function signatures

```rust
fn gc_content(seq: &[u8]) -> f64 { /* reads */ }
fn reverse_complement_into(seq: &[u8], out: &mut Vec<u8>) { /* writes out */ }
fn into_uppercase(v: Vec<u8>) -> Vec<u8> { /* takes ownership */ }
```

Read the signatures **as contracts**:

- `&[u8]` — "I will read your bytes; you keep them."
- `&mut Vec<u8>` — "I will write into your buffer; you keep the allocation."
- `Vec<u8>` — "Hand it over; I own it now."
...
![A fat pointer: &[u8] is two words on the stack — a pointer to the first byte and a length — viewing a sub-range of a Vec's heap allocation.](images/lec1-slice-into-vec.svg){fig-alt="Stack on the left contains v's Vec header and s's fat pointer (ptr + len = 4); heap on the right shows an 8-cell allocation owned by v with a green bracket marking the &v[2..6] range that s points into."}
...
Out-of-bounds slicing **panics at runtime** — the compiler can't catch it statically because the length isn't known at compile time.
...
![Three k=3 sliding windows over the same 7-byte Vec<u8>; each window is a &[u8] pointing into the same backing storage.](images/kmers.svg){fig-alt="A 7-cell row of DNA bytes ACGTACG with three coloured brackets below labelling the three k=3 windows ACG, CGT, GTA, each a &[u8] view into the same backing storage."}
...
The trick is `clear()`. It sets the Vec's length to zero, but it does *not* free the underlying allocation. The capacity stays the same. Next time the function fills the buffer, no allocator call is needed — the storage is right there from last time.

For a million reads, this is the difference between a million allocator round-trips and effectively zero. Allocation is one of the few things in Rust that has measurable per-call cost, so removing it from the hot path matters.
...
![Three states of the caller's Vec<u8> across one call: full from a previous call, empty after clear() but same allocation, full again after revcomp_into wrote into the existing storage.](images/revcomp-buf.svg){fig-alt="Three rows showing the same heap allocation across three states of a caller-owned Vec: filled (len 12, cap 64), cleared (len 0, cap 64), and refilled with the reverse complement (len 12, cap 64), with a yellow callout noting the heap allocation never moves."}

::: notes
The same heap block in all three rows. Top: 12 bytes from the previous call. Middle: `clear()` set the length to 0; the allocation is still there. Bottom: `revcomp_into` walked the input in reverse, complemented each base, and pushed into the existing storage — no allocator call, because the capacity was already 64.
...
fn quality_trim(qual: &[u8], min_score: u8) -> &[u8] {
    ...
    let mut start = 0;
    while start < qual.len() && qual[start] < threshold { start += 1; }
    let mut end = qual.len();
    while end > start && qual[end - 1] < threshold { end -= 1; }
    &qual[start..end]
}
```
...
![Two-pointer quality trim: start walks rightward and end walks leftward until both land on bases above threshold; the function returns &qual[start..end].](images/quality-trim.svg){fig-alt="A 10-cell strip of Phred+33 quality bytes with cells 0-2 and 8-9 shaded grey (trimmed) and cells 3-7 shaded green (kept); two green brackets mark final positions start=3 and end=8; faded dashed arrows arc inward from the initial positions 0 and 10."}
...
![A function signature on the left as the programmer writes it; on the right, the same signature with the implicit lifetime 'a drawn in on both the input and output reference.](images/lec1-lifetime-elision.svg){fig-alt="Top: the signature fn quality_trim(qual: &[u8], min: u8) -> &[u8]. Below: the same signature with the implicit lifetime 'a on both input and output, plus a dashed arrow linking input to output and the rule 'output lives no longer than the input'."}
...
![Three-row reference table comparing owned T, shared borrow &T, and mutable borrow &mut T across read, mutate, drop, and count.](images/lec1-ownership-table.svg){fig-alt="A table with rows T (owned), &T (shared borrow), and &mut T (mutable borrow), and columns Can read?, Can mutate?, Can drop?, How many?. T can read, mutate (if mut) and drop, exactly 1 owner. &T can read only, any number. &mut T can read and mutate, exactly 1. Footer: any number of readers or exactly one writer."}
