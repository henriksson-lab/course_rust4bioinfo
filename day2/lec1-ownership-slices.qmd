...
- Today's tools: ownership, `&T`, `&mut T`, slices (`&[u8]`), `Vec<T>`
:::
...
Without ownership rules, here's a classic C bug:

```c
char* buf = malloc(1000);   // ask the system for 1000 bytes
// ... use buf ...
free(buf);                  // give the memory back
// ... 50 lines later, someone else still has a pointer to it:
buf[0] = 'X';               // CRASH, or silent corruption
                            // (the memory may have been handed out
                            //  to a completely different variable by now)
```

This single bug class &mdash; *use-after-free* and its cousin *double-free* &mdash; is responsible for roughly **70%** of security vulnerabilities in C and C++ software. Microsoft and Google's Chromium team have each published this number. Rust's ownership rule prevents the entire class at compile time &mdash; no runtime check, no garbage collector. The rest of this lecture is how that works.
...
Copying a `u8` or a `u64` is one CPU instruction &mdash; there's no performance difference between "move" and "copy" for these tiny types. So Rust copies them for you automatically, as a convenience. You'd be annoyed if you had to clone every integer just to use it twice.

Primitive types implement [`Copy`](https://doc.rust-lang.org/std/marker/trait.Copy.html). Assigning them duplicates the value instead of moving.
...
Rule of thumb: types that are **small and own no heap memory** are `Copy`. So `u8`, `i32`, `f64`, `bool`, `char`, fixed-size arrays of those.

`Vec<u8>`, `String`, `File` &mdash; **not** `Copy`. They own a resource.

::: notes
If everything moved on assignment, primitives would be unusable — you couldn't even write `let r = q; if r >= 20 && q >= 20`. Rust handles this with a trait called `Copy`. Types that implement `Copy` are duplicated on assignment instead of moved.
...
Imagine you're iterating over a `Vec<u8>` (a sequence) and, inside the loop, you decide to *push* a new base onto the same Vec. If the Vec grows past its current capacity, the system has to hand it a bigger chunk of memory somewhere else &mdash; and the reference your loop is holding now points at the *old* place, which has already been handed back. Read through that reference and you get either garbage or whatever data happens to live there now.

```rust
let mut v = vec![b'A', b'C', b'G'];
let first = &v[0];          // shared borrow into v's heap memory
v.push(b'T');               // may move v to a bigger chunk
                            // -> first would point at the old, freed memory!
println!("{}", first);      // (in C++, undefined behaviour and silent corruption)
```

Rust rejects this at compile time. `first` is a `&u8` into v's buffer; `v.push` needs `&mut v`; the two cannot coexist.
...
let mut v = Vec::with_capacity(100);       // empty, pre-allocated for 100
v.push(b'A');                              // append
...
::: notes
[`Vec`](https://doc.rust-lang.org/std/vec/struct.Vec.html) is the standard growable array. It is the workhorse of Rust just as `list` is the workhorse of Python.

`Vec::new` makes an empty Vec with no heap allocation yet. The first push will allocate. `Vec::with_capacity(n)` pre-allocates room for n elements — useful when you know the rough size up front, because it avoids the geometric "double the capacity" reallocations as you fill it.

`push` appends. `clear` sets the length to zero but keeps the allocation — that's the trick we'll use in the buffer-reuse exercise. `reserve` tops up capacity if needed; it's a no-op if you already have room.
...
The empty form `&seq[..]` is the whole thing — a slice covering the entire Vec. This is how you turn a `Vec<u8>` into a `&[u8]` when calling a function that expects one. (In many cases, Rust does this conversion implicitly via a feature called Deref coercion, but the explicit form is always available.)
...
What would your code look like *without* elision? Every function that returns a borrow would need an explicit lifetime parameter, like `fn first_word<'a>(s: &'a str) -> &'a str`. Multiply that by every function in every project and the noise becomes overwhelming &mdash; so the compiler infers the lifetimes for you in the common cases.

For one-input, one-output reference signatures, the compiler infers what's safe:
...
In ~90% of cases the compiler can infer the lifetimes from the function signature: a single input borrow gives the output borrow the same lifetime; multiple input borrows with `&self` give the output borrows from `self`. When elision can't decide, the compiler asks you to write the lifetime explicitly. That's rare enough that you can ignore it today.
...
::: {.incremental}
- **E0382: borrow of moved value** &mdash; you used `v` after `let w = v`. Fix: pass `&v` instead, or reorder.
- **E0502: cannot borrow as mutable / immutable** &mdash; you have a `&` and a `&mut` overlapping. Fix: narrow the scope of one borrow, or use an index.
- **E0507: cannot move out of borrowed content** &mdash; you tried to take ownership from behind a `&T`. Fix: copy the value, clone, or change the API.
- **E0596: cannot borrow as mutable** &mdash; the binding isn't `mut`. Fix: `let mut v = ...`.
:::

::: notes
You will see all four of these by the end of the afternoon. They are not bugs — they are the compiler protecting you.

E0382 is the move-then-use error. Often you didn't actually mean to take ownership; passing `&v` instead solves it.

E0502 is the borrow overlap. The fix is to make sure the shared borrow ends before the mutable borrow begins, often by introducing a `{ }` block or reordering operations.
...
E0596 is the smallest one: you forgot the `mut` keyword on a `let` binding.
