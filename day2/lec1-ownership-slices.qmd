...
```c
char input[8];                  // a buffer that holds 8 bytes
strcpy(input, "ACGTACGTACGTACGT");  // writes 16 bytes!
// the extra 8 bytes overwrite whatever happened to sit
// just past `input` in memory — could be another variable,
// could be the function's return address, could be anything.
```

C lets you write past the end of an array silently. The first bytes go where they should; the rest **corrupt adjacent memory**. The program either crashes (if you're lucky), prints garbage (if not), or — if the overwritten region is a function pointer — runs whatever code the attacker chose.

![A 16-byte write into an 8-byte buffer spills into the next allocation; that adjacent allocation might be a function-return address.](images/lec1-buffer-overrun.svg){fig-alt="A small diagram: a buffer of 8 cells labelled 'input' with the bytes ACGTACGT, followed by 8 more cells labelled 'adjacent variable' or 'return address'. A red arrow shows the strcpy writing through the buffer boundary into the adjacent region, with the labels turning red."}

**Where this bites the web**: every browser, every JavaScript engine, every PDF reader has been hacked through this pattern. CVE catalogues are full of "out-of-bounds write" entries.

**Where this bites bioinformatics**: visualisation tools (genome browsers, alignment viewers) parsing untrusted BAM/VCF/GFF data are exactly the kind of C code where one malformed input record can corrupt memory. The bug shows up as a viewer crash; the security implication is the same as anywhere else.

Rust's slice indexing checks bounds on every access — out-of-range index panics cleanly with a clear message instead of silently corrupting memory.
...
```python
v = [[1, 2], [3, 4]]
first = v[0]         # `first` IS the same list object as v[0]
v.append([5, 6])     # grows v — but [1, 2] is still the same object
v[0].append(99)      # mutate through the original
print(first)         # [1, 2, 99] — `first` silently changed
```

Because Python lists hold *references* to mutable objects, two variables can quietly point at the same underlying data. Changing one changes the other, with no warning. The bug is famous enough to have its own anti-pattern name — *aliasing*.

The same thing happens with pandas DataFrames (the chained-assignment SettingWithCopyWarning), with NumPy views, with R reference semantics.
...
The signature returns `&[u8]` — a borrow of the input, not a fresh allocation. This is the right shape because trimming never needs to produce new bytes; it just narrows the range of existing ones. Returning a slice is a fixed tiny amount of work regardless of how long the input is.
