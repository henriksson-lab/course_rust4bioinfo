...
The **stack** is a fast, last-in-first-out region used for function calls. Every time you call a function, a fresh frame is pushed on top; when it returns, the frame is popped.
...
::: {.fragment}
![Step 2: main() calls compute(seq) — a new frame is pushed on top.](images/lec1-stack-frames-step2.svg){fig-alt="The stack now has two frames: main() at the bottom and gc_count() on top, holding its local variables."}
:::

::: {.fragment}
![Step 3: compute() returns — its frame is popped; main() continues with the result.](images/lec1-stack-frames-step3.svg){fig-alt="The stack has returned to a single main() frame; the gc_count() frame has been popped and the result is held in main's local n."}
:::
...
In C or Rust, a small value like `42` lives directly in its 4-byte slot. In Python, the variable is a pointer; the value is wrapped in a small heap-allocated object that also stores a type tag and a reference count.
