...
![](images/lec1-computer-overview.svg){#fig-overview width="85%" fig-alt="A motherboard rectangle containing labelled components: a CPU box with caches at upper centre, two narrow DDR4 RAM modules to its right, an NVMe SSD and a network card on the far right, a long GPU board across the lower half, and a power supply on the left. Thick and thin lines connect components, labelled memory bus 100 ns, PCIe 10 microseconds, and PCIe NIC varies. Schematic of a typical desktop or laptop: motherboard with CPU socket, RAM DIMM slots, NVMe storage, GPU, network card, and PSU, connected by buses of varying speed."}

![A real motherboard — the same components as the schematic, in the wild. *Photo: Darkone, [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:ASRock_K7VT4A_Pro_Mainboard.jpg), CC BY-SA 2.5.*](images/lec1-motherboard-photo.jpg){width="70%" fig-alt="Photograph of an ASRock K7VT4A Pro motherboard top-down, showing the CPU socket, RAM slots, chipset heatsink, PCI and AGP expansion slots, capacitors, and rear I/O ports; provided as a real-world complement to the abstract schematic above."}
...
::: {.fragment}
![Step 1: CPU asks for byte at offset 5.](images/lec1-cache-line-step1.svg){fig-alt="A horizontal strip of 64 small square cells representing a region of RAM. The byte at offset 5 is highlighted as the byte the CPU has requested."}
:::

::: {.fragment}
![Step 2: the whole 64-byte line is fetched into the cache (one slow RAM trip).](images/lec1-cache-line-step2.svg){fig-alt="The full 64-byte cache line is shown being copied from RAM into the CPU cache as a single block, in one slow RAM access."}
:::

::: {.fragment}
![Step 3: nearby byte reads are cache hits, fast and free.](images/lec1-cache-line-step3.svg){fig-alt="Subsequent reads from adjacent bytes within the same 64-byte line are served from the cache, marked as fast cache hits."}
:::
...
When the OS launches your program, it sets up four distinct regions of memory for it.

- **Code** — the program's instructions, loaded from your binary
- **Data** — fixed values known before the program starts (constants, string literals)
- **Heap** — a flexible pool the program asks for as it runs
- **Stack** — a small fast region used for function calls and local variables
...
::: notes
Every running program sees this layout, regardless of language. The OS gives each process its own virtual address space — a private playground that looks the same shape every time. Code and constants at the bottom, the heap growing up, the stack growing down. The next two slides zoom in on the two regions you, as a programmer, actually interact with: the stack and the heap.
:::
...
The **stack** is a fast, last-in-first-out region used for function calls. Every time you call a function, a fresh **stack frame** (the slice of stack belonging to one call: its arguments and local variables) is pushed on top; when it returns, the frame is popped.
...
::: {.fragment}
![Step 1: main() running — one frame on the stack.](images/lec1-stack-frames-step1.svg){fig-alt="A stack diagram with a single frame for main() containing the local variable seq."}
:::

::: {.fragment}
![Step 2: main() calls gc_count(seq) — a new stack frame (the slice of stack belonging to one call) is pushed on top.](images/lec1-stack-frames-step2.svg){fig-alt="The stack now has two frames: main() at the bottom and gc_count() on top, holding its local variables."}
:::

::: {.fragment}
![Step 3: gc_count() returns — its frame is popped; main() continues with the result.](images/lec1-stack-frames-step3.svg){fig-alt="The stack has returned to a single main() frame; the gc_count() frame has been popped and the result is held in main's local n."}
:::
...
## Boxed vs unboxed (boxed = wrapped in a heap object) — what the value actually costs

In C or Rust, a small value like `42` lives directly in its 4-byte slot. In Python, the variable is a pointer; the value is wrapped in a small heap-allocated object that also stores a **type tag** (a label saying "this is an int, not a string") and a **refcount** (count of how many places refer to this object).

![Same `42`, very different memory: C/Rust store the 4 bytes directly; Python wraps it in a heap-allocated object with type tag and refcount.](images/lec1-boxed-vs-unboxed.svg){fig-alt="Two columns. Left: C/Rust shows a single 4-byte box holding 42 directly. Right: Python shows a small pointer on the stack pointing into the heap to a larger object containing a type tag, a reference count, and the value 42."}

- One Python integer is tens of bytes; one C/Rust integer is 4 bytes
- Each Python value needs a pointer hop — bad news for the cache
- A million-int list: ~60 MB in Python vs ~4 MB in C/Rust

::: notes
This is the price of Python's convenience. Because Python lets you write x = 42 today and x = "hello" tomorrow and the runtime has to handle either, every value carries a type tag and a refcount. The smallest integer in Python is about 28 bytes — seven times the size of a C int. Multiply by the number of vectors in a genomics pipeline; this is where Python loses an order of magnitude.
...
- **Kernel** (the privileged core of the OS that can touch hardware) — runs the scheduler, file system, network stack
- **User space** (where ordinary programs run, with no direct hardware access) — your browser, your Python script, your Rust tool
- **Syscall** (a controlled jump from user space into the kernel) — the only legal way for your program to ask the kernel to do something
...
When your Python code does:

```python
f = open("data.fa")
```

what actually happens is: the Python runtime makes a syscall — `open("data.fa", O_RDONLY)` — which is a controlled jump from user space into the kernel. The kernel checks permissions, locates the file, returns a file descriptor (a small integer the kernel hands back so your program can refer to the file later). Then control comes back to your code.
