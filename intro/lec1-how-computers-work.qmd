...
**To fully understand Rust, you need an idea of how a computer actually works.**

What we'll cover:

- The big picture: CPU, RAM, registers, cache, storage
- The memory hierarchy and the memory wall
- Cache lines and why sequential access wins
- How a program lays out memory: code, static data, heap, stack
- Stack frames and function calls
- Boxed vs unboxed values
- The OS, syscalls, and the kernel/user-space split
- What happens when you run a program, end to end
- Why this matters for bioinformatics performance
...
![The three things you need to recognise: CPU, RAM, storage. *Photo: Darkone, [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:ASRock_K7VT4A_Pro_Mainboard.jpg), CC BY-SA 2.5.*](images/lec1-motherboard-annotated.svg){width="80%" fig-alt="An ASRock K7VT4A Pro motherboard photo with three overlaid annotation arrows labelling only the CPU socket on the left, the RAM DIMM slots on the upper right, and the storage (IDE/SATA) connectors on the lower right."}

::: notes
The same components as the schematic, in the wild. You only need to recognise three things: the CPU socket, the RAM slots, and the storage connectors. Everything else on the board — chipset, capacitors, expansion slots, I/O ports — is plumbing. Every machine you own (phone, laptop, server) is a variant of this same three-box layout.
:::
...
## The CPU — arithmetic + decisions {.smaller}
...
![From registers to RAM in nanoseconds — what the CPU actually waits on. *Each arrow is a cache miss — the requested byte wasn't in the faster level above, so the CPU has to fetch from the next level down.*](images/lec1-latency-ladder.svg){width="90%" fig-alt="A horizontal flow chart from register → L1 → L2 → L3 → RAM with each transition labelled with its access latency in nanoseconds. Each level coloured from green (fast) to red (slow)."}

::: {layout-ncol=2}
![A CPU die — registers, ALU, L1/L2/L3 all live on the same silicon chip.](images/lec1-cpu-mini.svg){fig-alt="A small schematic of a CPU package showing 4 cores in a 2x2 grid, each with private L1 cache, L2 cache, and a shared L3 cache at the bottom."}

![A RAM module — separate chip board sitting in a DIMM slot.](images/lec1-ram-mini.svg){fig-alt="A small schematic of a DDR4 RAM stick: 8 memory chips arranged in a row on a PCB with edge connector at the bottom."}
:::
...
## A cache line — 64 bytes at a time {.smaller}
...
![Step 1: CPU asks for byte at offset 5.](images/lec1-cache-line-step1.svg){width="70%" fig-alt="A horizontal strip of 64 small square cells representing a region of RAM. The byte at offset 5 is highlighted as the byte the CPU has requested."}

::: notes
The CPU never fetches a single byte from RAM. It always fetches a whole 64-byte aligned block — the cache line — even if you only asked for one byte. We are about to see what the next step looks like.
:::

## A cache line — what step 2 looks like {.smaller}

![Step 2: the whole 64-byte line is fetched into the cache (one slow RAM trip).](images/lec1-cache-line-step2.svg){width="70%" fig-alt="The full 64-byte cache line is shown being copied from RAM into the CPU cache as a single block, in one slow RAM access."}

::: notes
The whole 64-byte line lands in the cache as one block. Each fetch costs about a hundred nanoseconds. That is the punch line: one slow RAM trip pays for 64 bytes' worth of subsequent access.
:::

## Cache lines — and why sequential access wins

![Step 3: nearby byte reads are cache hits, fast and free.](images/lec1-cache-line-step3.svg){width="70%" fig-alt="Subsequent reads from adjacent bytes within the same 64-byte line are served from the cache, marked as fast cache hits."}
...
::: {layout-ncol=3}
![DDR3 RAM stick.](images/lec1-ram-photo.jpg){fig-alt="Photo of a DDR3 RAM module — a long thin green PCB with memory chips along it and a notched gold edge connector at the bottom."}

![NVMe SSD (M.2 form factor).](images/lec1-ssd-photo.jpg){fig-alt="Photo of an NVMe M.2 SSD — a small green PCB about the size of a stick of gum with a single controller chip and 1-2 memory chips."}

![Hard disk drive with platter visible.](images/lec1-hdd-photo.jpg){fig-alt="Photo of a hard drive with the cover removed, showing the silver platter and the read/write arm hovering over it."}
:::

*Photos (left to right): Alan Lorenzo, [DDR3 RAM](https://commons.wikimedia.org/wiki/File:Memoria_DDR3_Patriot_Viper_PVS34G1333LLK_-_2.JPG), CC BY-SA 3.0; Wikimedia user `-stk`, [Toshiba M.2 SSD](https://commons.wikimedia.org/wiki/File:Toshiba_M2_2280_256_GB_SSD_front.jpg), CC BY-SA 4.0; Evan-Amos (Vanamo Media), [laptop HDD exposed](https://commons.wikimedia.org/wiki/File:Laptop-hard-drive-exposed.jpg), CC BY-SA 3.0.*
...
## A running program in memory {.smaller}

The OS sets up the same shape for every process: **code** at the bottom, **heap** (grows up), **stack** (grows down), with a big empty gap in between.

![Every process gets the same private memory shape.](images/lec1-program-memory-layout.svg){#fig-layout width="50%" fig-alt="A tall rectangle representing one process's virtual address space, divided into horizontal bands: at the bottom '.text (machine code, read-only)', then a 'heap' band labelled 'big data lives here' with an upward arrow showing it grows toward higher addresses, then a big empty middle marked as unused virtual address space, then a 'stack' band labelled 'variables inside functions live here' with a downward arrow, and a 'kernel space' band at the top."}
...
## Stack frames — push on call, pop on return {.smaller}
...
::: notes
The stack grows and shrinks one frame per function call. A stack frame is the slice of stack belonging to one call — its arguments and local variables. Right now only main() is on the stack, holding its local variable seq. Next slide: what happens when main calls gc_count(seq).
:::

## Stack frames — push on call, pop on return (step 2) {.smaller}
...
::: notes
A new frame is pushed when main calls gc_count, and popped when gc_count returns — no allocator involved, just a pointer adjustment.
...
![Tidy stack frames on the left; the heap on the right holds allocations of any size, with pointers from stack-frame variables reaching into them.](images/lec1-heap-allocation.svg){width="80%" fig-alt="Two-column diagram. Left column shows a blue 'Stack' region containing three small fixed-size frames labelled process_read, parse_fastq and main, each with local variables. Right column shows a wider orange 'Heap' region containing several different-sized white rectangles representing allocations of varying sizes (a small String, a Vec<u8> of 1 KB, a Box<i64>, a 12 MB Vec<Record>, a HashMap node, a Box<[u8; 64KB]>) interspersed with small dashed 'free' gaps. Two blue pointer arrows from variables inside the stack frames point into allocations inside the heap. A box at the bottom shows the C malloc/free and Rust Box::new allocator API. Annotations on the right read 'any size, any lifetime', 'you decide when to free', 'more bookkeeping than the stack'."}
...
<object type="image/svg+xml" data="images/lec1-syscall-animated.svg" width="80%" aria-label="An animated diagram (auto-plays) showing the steps from your program calling open() and read() down through the kernel to the disk controller and back: the request flows down, the bytes flow back up over ~5 seconds, with stage labels appearing one at a time.">
</object>
:::
...
## Why latency matters — clinical turnaround

Same machine speed, very different acceptable wait times depending on the question:

| Scenario | Acceptable turnaround |
|---|---|
| ER: suspected sepsis — which antibiotic? | < 1 hour |
| ICU: drug-resistance variant calling | hours |
| Rare-disease workup: full WGS analysis | days |
| Cancer panel: tumour mutation report | days to a week |
| Public-health outbreak: SARS-CoV-2 surveillance | days |
| Cohort study / GWAS: hundreds of genomes | weeks-months |
| Research method development | months |

The bottom rows are tolerant of slow code. The top rows are NOT — every second your pipeline takes is a second the clinical decision is waiting on you. Rust's "compile-time-free of a class of bugs, runtime-fast" combination is most valuable up there.

::: notes
Tie this back to the latency ladder: nanoseconds and microseconds matter at the machine level; hours and days matter at the human-decision level. The point is that "fast code" matters more in some scenarios than others — and Rust pays off most where it matters most.
:::
