...
![The same components as the schematic, in the wild — arrows show where each part lives on a real motherboard. *Photo: Darkone, [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:ASRock_K7VT4A_Pro_Mainboard.jpg), CC BY-SA 2.5.*](images/lec1-motherboard-annotated.svg){width="80%" fig-alt="An ASRock K7VT4A Pro motherboard photo with overlaid annotation arrows labelling the CPU socket, RAM DIMM slots, PCI/AGP expansion slots, chipset heatsink, capacitors, and rear I/O ports."}
...
![Real RAM — this is where the running program lives. Each byte has an address; the OS gives each process its own private address space. *Photo: Alan Lorenzo, [Wikimedia](https://commons.wikimedia.org/wiki/File:Memoria_DDR3_Patriot_Viper_PVS34G1333LLK_-_2.JPG), CC BY-SA 3.0.*](images/lec1-ram-photo.jpg){width="50%" fig-alt="Photo of a DDR3 RAM module."}
...
![](images/lec1-syscall-animated.svg){width="80%" fig-alt="An animated diagram (auto-plays) showing the steps from your program calling `open()` and `read()` down through the kernel to the disk controller and back: the request flows down, the bytes flow back up over ~5 seconds, with stage labels appearing one at a time."}
