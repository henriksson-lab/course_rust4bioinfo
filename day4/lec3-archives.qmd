...
The algorithm inside `.zip` and `.gz` is called **DEFLATE** (a lossless compression algorithm — the standard one used by gzip and ZIP). It combines two plain-English ideas, each with a fancy name:

- **find repeats** — also known as **LZ77** — when a substring already appeared in the recent window, write a **back-reference** (an offset and length pointing into the recent past) instead of the bytes themselves
- **give common letters shorter codes** — also known as **Huffman coding** — frequent bytes (`A`, `C`, `G`, `T` in DNA) get short bit-codes, rare ones get longer codes

Worked example — find-repeats on a sliding window:
...
::: {.fragment}
Disk-read time on a single 12 GB lane (200 MB/s disk, 300 MB/s gzip decode):
