...
/// Compute per-record (name, length, GC fraction) from any FASTA reader.
pub fn fasta_stats<R: BufRead>(reader: R) -> std::io::Result<Vec<ContigStat>> {
    let mut _reader = fasta::io::Reader::new(reader);
    // TODO:
    //   1. iterate _reader.records()
    //   2. for each Result, ? to short-circuit on error
    //   3. compute length and gc fraction from record.sequence().as_ref()
    //   4. push ContigStat onto a Vec; return Ok(vec)
    Ok(Vec::new())
}
