...
- **No statistical transforms** — no `geom_smooth` / `sns.regplot`, no `stat_summary`, no kernel density / `sns.kdeplot`.
- **No grammar of graphics** — no `aes(fill = species)` / `hue=` in seaborn, no auto-legend from a category column.
- **No tidy data input** — you wrangle the data yourself before drawing; no DataFrame in / plot out.
- **Limited types** — no boxplot / `plt.boxplot`, no violin / `sns.violinplot`, no heatmap / `sns.heatmap` as a first-class series.
:::
