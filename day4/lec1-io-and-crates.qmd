...
There is also [`fs::read`](https://doc.rust-lang.org/std/fs/fn.read.html) — returns `Vec<u8>` instead of a `String`, no **UTF-8** (the modern text encoding for any character; **ASCII** is the older 1-byte-per-character subset) check.
...
We need to **stream** (process the file piece-by-piece, never holding the whole thing in RAM at once) — load a small piece, process it, drop it, load the next.
...
A **buffer** here is a small fixed-size byte array (~64 KB) that the standard library refills from disk in one syscall.

```rust
let file   = File::open("reads.fastq")?;          // open the file
let reader = BufReader::new(file);                // wrap in a 64 KB buffer
for line in reader.lines() { /* one line at a time */ }
```

::: notes
The file on disk is enormous. The buffer in RAM is small — 64 KB by default. The iterator yields one line at a time, and as soon as you stop using it, the line is dropped.

Doubling the file size does not change the memory used by the program. Only the disk reads multiply.
:::

## `BufReader` visualised
...
A **trait** (shared interface — like Python's `__iter__` lets `for` work over lists, files, and generators) is a set of method signatures that many types can implement. The standard library defines small traits and every readable thing implements them:
...
Code that takes a value implementing `BufRead` works with a file, an in-memory byte string, a gzip stream — anything that implements the trait. We will see the generic syntax (`R: BufRead`) on a later slide.
...
The picture on the next slide shows the nested boxes.
:::

## File wrapped in BufReader
...
[`.lines()`](https://doc.rust-lang.org/std/io/trait.BufRead.html#method.lines) yields one `Result<String>` per line, lazily. Use it for BED, TSV, plain FASTA headers.
...
An **iterator** is an object that produces one value at a time on demand, like a Python generator.
:::
...
::: notes
The picture makes the point: at any moment ~64 KB is in memory; everything else is on disk. The yielded `String` is owned by you; the next iteration drops it before fetching the next line.
:::

## `Result` propagation — `?` is your verb

```rust
fn read_fasta(path: &Path) -> Result<String, io::Error> {
    let mut file = File::open(path)?;                // fail -> return Err
    let mut s = String::new();
    file.read_to_string(&mut s)?;                    // fail -> return Err
    Ok(s)
}
```
...
## `?` on an `Option` — promoting "missing" to an error

Sometimes you get an `Option` (the value might be missing) but your function returns a `Result`. Convert with `ok_or_else`:

```rust
fn first_header(path: &Path) -> Result<String, io::Error> {
    let file   = File::open(path)?;
    let reader = BufReader::new(file);
    reader.lines().next()                            // Option<Result<String>>
          .ok_or_else(|| io::Error::other("empty FASTA"))?   // -> Result
}
```

`ok_or_else` takes a **closure** (anonymous function — the `|| ...` bit) that builds the error only if the `Option` is `None`.

::: notes
This is a follow-up slide for students who want the slightly fancier version. `next()` on a lines iterator returns `Option<Result<String>>` — `None` if the file is empty, `Some(Ok(line))` if it has a first line, `Some(Err(e))` if reading failed.

`ok_or_else` is the bridge: it turns `None` into `Err(...)` and `Some(x)` into `Ok(x)`. The closure is only called on the `None` branch, so building the error is lazy.

Read it as "if it's there, fine; otherwise, build this error". The trailing `?` then propagates the resulting `Result` like any other.
:::
...
For each line in `[dependencies]`, Cargo:

- **downloads** the dep from `crates.io`
- **caches** it in `~/.cargo` (shared across all your projects)
- **links** it statically into your **binary** (the single compiled program file)

::: notes
Three lines of TOML say "this project uses these crates". On `cargo build`, Cargo reads the manifest, contacts `crates.io` for compatible versions, downloads them once into `~/.cargo/registry`, builds them, and links them into your binary.

The cache is shared across all your projects on this machine. First time slow; subsequent builds free. The resulting binary contains the machine code for every dependency, statically linked — no "missing shared library" errors at deploy.
:::

## Cargo.toml to running binary
...
::: notes
You add one line to Cargo.toml. Cargo handles fetching, version resolution, caching, and linking.
:::
...
::: notes
Patch = bug fix, drop-in safe. Minor = new feature, backward compatible. Major = breaking change. Cargo's `"1.4.2"` defaults to `^1.4.2` — anything in the 1.x series, ≤ 2.0.0.
:::
...
## How to learn a new crate — five steps

1. **Search crates.io.** Downloads count is a rough proxy for community trust.
2. **Read the package page.** Check license, last-updated date, repository link.
3. **Open docs.rs.** Top-of-page module docs usually show a runnable example.
4. **Paste the example into your project.** Get the canonical example working before customising.
5. **Add it to `Cargo.toml`** and run `cargo build`.

::: notes
The whole discovery-to-use loop. First signal when searching is "what's popular": a package nobody downloads has no track record. Second, on the package page: is it alive, is the license compatible. Third, the top of the docs — if there is no example, that is a red flag.

Once the example runs, paste it in, edit until it does what you need. Most crates you will ever use, you find this way.
:::

## A docs.rs landing page
...
::: notes
This is where you will spend most of your time once you have picked a crate.
:::

## `noodles` — streaming records, step 1
...
::: notes
You give a `Reader` something that implements `BufRead`. The reader allocates an internal buffer but does not read anything yet.
:::

## `noodles` — streaming records, step 2
...
::: notes
The reader parses one record at a time and yields it to your loop body.
:::

## `noodles` — streaming records, step 3
...
::: notes
When the loop body finishes one iteration, the record is dropped — its memory goes back to the allocator before the next record is read.

Program memory is constant in the file size. A 3 GB FASTQ takes the same RAM as a 3 KB one. Only disk reads multiply. `noodles_fasta`, `noodles_fastq`, `noodles_bam`, `noodles_vcf` and the dozen others all share this design.
:::
...
fn count_records(reader: BufReader<File>) -> std::io::Result<usize> {
...
[`fasta::io::Reader::new`](https://docs.rs/noodles-fasta/latest/noodles_fasta/io/struct.Reader.html) wraps the buffered file. [`.records()`](https://docs.rs/noodles-fasta/latest/noodles_fasta/io/struct.Reader.html#method.records) yields `Result<Record>`.
...
The function takes a concrete `BufReader<File>` here so the type signature is easy to read. On the next slide we make it generic so the same function will also accept test bytes — but the body of the function does not change.
...
::: notes
File = raw bytes from disk. BufReader = chunked reads with a 64 KB buffer. noodles Reader = FASTA semantics: headers, wrapped sequences, validation. Each layer does one job.
:::

## Same function, generic over the reader

A **generic** function is one that works for any type that meets a stated requirement. The requirement here — `R: BufRead` — is called a **trait bound**.
...
Same body. Now both calls compile:

```rust
count_records(BufReader::new(File::open("sample.fa")?))?;        // real file
count_records(Cursor::new(b">chr1\nACGT\n>chr2\nTTGCAA\n"))?;    // test bytes
```

::: notes
Same body, one extra annotation in the signature. `<R: BufRead>` says "this function accepts any type `R` as long as `R` implements the `BufRead` trait". `BufReader<File>` qualifies. `Cursor<&[u8]>` qualifies. So do gzip-decoded streams and many other things.

The compiler generates a specialised copy of the function for each concrete `R` you use — that process is called **monomorphisation**: one generic blueprint, many compiled copies, one per concrete type. The runtime cost is zero because each call is a direct function call to the specialised copy.

This is why trait-based I/O is the foundation of testing in Rust. Real I/O in production, in-memory bytes in tests, the same function body.
:::
...
for result in reader.records() {
    ...
    for &base in seq {                          // &base = give me the byte
        if base == b'G' || base == b'C'
            || base == b'g' || base == b'c' {
            gc_count += 1;
        }
    }
    let gc = gc_count as f64 / length as f64;
}
```
...
A plain `for` loop with a counter is shown here for clarity. The same thing can be written as `seq.iter().filter(|&&b| matches!(b, b'G' | b'C' | b'g' | b'c')).count()` — the `&&b` is "the filter gives a reference to a reference; deref twice to get the byte" — but if you are seeing those tokens for the first time, the explicit loop is easier to read.

The accompanying picture (next slide) shows the `Record` struct: heap-owned name and sequence, dropped when the loop iteration ends.
:::
...
::: notes
The record owns its data; dropping it frees the heap allocations. Borrowed slices from `.name()` and `.sequence()` are valid only while the record is alive.
:::
...
::: notes
Constant memory; one record in flight at a time. The same shape applies to any record-level transform you write today.
:::

## Modules — splitting one crate across files

A real project does not live in one `main.rs`. Three keywords carry the system:

- **`mod`** declares a child module — `mod parser;` tells the compiler "look for `parser.rs`"
- **`pub`** controls visibility — without `pub`, items are private to the module
- **`use`** brings names into scope so you can write `parse_record(...)` instead of `crate::parser::parse_record(...)`
...
`pub` controls what is visible outside a module. The default is private — like immutability, the careful choice has to be opted into.
:::
...
::: notes
mod declares a child module; pub controls what leaks out; use brings a name into scope.
:::
