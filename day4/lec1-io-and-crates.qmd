...
[`.lines()`](https://doc.rust-lang.org/std/io/trait.BufRead.html#method.lines) is an **iterator** (an object that produces one value at a time on demand, like a Python generator) that yields one `Result<String>` per line. It is **lazy** — nothing is read until you ask for the next item. Use it for line-oriented formats: BED, TSV, plain FASTA headers.
...
::: notes
The whole discovery-to-use loop on one slide. The screenshot is the docs.rs landing page — that page is where you will spend most of your time once you have picked a crate.

The first signal when searching is "what's popular": a package nobody downloads has no track record. The second signal, on the package page, is whether it is alive and whether the license is compatible. The third is the top of the docs — if there is no example, that is a red flag.

Once the example runs, paste it in, edit until it does what you need, then commit. Most crates you will ever use, you find this way.

(Click-through appendix slides — one screenshot per step — are available if you want to dwell on any one of these.)
:::
...
for result in reader.records() {
    ...
    for &base in seq {                          // &base = "give me the byte, not the reference"
