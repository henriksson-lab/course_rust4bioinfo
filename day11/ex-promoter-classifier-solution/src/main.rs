//! Day 11 — Exercise 7: promoter classifier. Reference solution.
...
impl<B: Backend> PromoterNet<B> {
    pub fn new(device: &Device<B>, kernel: usize, channels: usize) -> Self {
        Self {
            conv: Conv1dConfig::new(4, channels, kernel).with_bias(false).init(device),
            fc:   LinearConfig::new(channels, 1).init(device),
        }
    }

    pub fn forward(&self, x: Tensor<B, 3>) -> Tensor<B, 2> {
        let h = relu(self.conv.forward(x));        // [N, C, L-k+1]
...
fn make_dataset<B: Backend>(
    ...
    for _ in 0..n_per_class { seqs.push(planted_promoter(&mut rng, l, "TATAAA", 10)); labels.push(1.0); }
    for _ in 0..n_per_class { seqs.push(random_seq(&mut rng, l));                    labels.push(0.0); }
...
mod tests {
    ...
    fn forward_shape() {
        ...
        assert_eq!(model.forward(x).dims(), [4, 1]);
    }
    ...
    fn one_train_step_runs() {
        ...
        let (_, loss) = train_step(model, &mut optim, 1e-2, x, y);
