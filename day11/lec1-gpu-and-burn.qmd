...
Mathematically, a rank-2 tensor of shape $[R, C]$ is the matrix $X \in \mathbb{R}^{R \times C}$ with entries $X_{i,j}$ for $0 \le i < R$, $0 \le j < C$. Element-wise addition and the matrix-vector product are the workhorse operations:
