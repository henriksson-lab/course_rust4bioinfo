...
A **race condition** [two threads touching the same memory, at least one writing, with no coordination] is the bug class Rust's `Send` and `Sync` prevent.
...
spawn(|| { for _ in 0..1_000_000 { total += 1; } });
println!("{total}");   // expected 2_000_000 — actually ~1.0M to 2.0M
```

`total += 1` is **three** CPU ops: read, add, write. If two threads interleave them, one write silently loses.
...
Handing an item to a worker isn't free. **Per-task overhead** [the bookkeeping rayon does per item, typically a few microseconds] has to be smaller than the per-item work, or you slow down.
...
**Rule of thumb:** <1 µs/item hurts; ~10 µs marginal; >100 µs (per-read alignment, per-record GC) is a clear win.
...
![Ideal linear (grey) vs observed sublinear (green): the gap grows with core count.](images/lec1-linear-vs-sublinear-scaling.svg){fig-alt="Bar chart with 4 groups (1, 2, 4, 8 cores). Each group has two bars side by side: 'ideal linear' (grey, heights 1, 2, 4, 8) and 'observed sublinear' (green, heights 1, 1.9, 3.7, 7.2). Dashed line above each green bar shows where linear would land."}

- **Ideal linear** — textbook: N cores → N× speedup.
- **Sublinear** [real speed-up, but less than N×] — Amdahl's law + memory-bandwidth contention.

For per-record bioinformatics, 7-8× on 8 cores is healthy. Stop tuning within 20% of ideal.

::: notes
Some part of every program is unavoidably serial, so doubling cores never quite doubles speed. Threads also compete for the same path to memory, so even fully parallel work doesn't always scale linearly.
:::
