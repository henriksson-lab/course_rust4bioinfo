...
Would you trust this still works after a small edit?

```python
# Python — what does this do?  Will the next person to edit it understand?
def parse_fastq_id(s):
    m = re.match(r'^@([^\s:]+):(\d+):([^:]+):(\d+):(\d+):(\d+):(\d+)\s+([12]):([YN]):(\d+):([ACGTN+]+)$', s)
    return {f: m.group(i+1) for i, f in enumerate(['inst','run','fc','lane','tile','x','y','read','filt','ctrl','idx'])}
```

This works today. What happens when next year someone adds a new field, or the sequencer firmware changes the format slightly? Without tests, you find out months later when a downstream report comes back wrong.

Tests catch the silent regressions you didn't expect. If `gc_content` returns 0.5 for `ACGT` today and someone "optimises" it next year so it now returns 0.4 because they changed how N bases are counted — your downstream pipeline silently produces different numbers. A test that asserts `gc_content(b"ACGT") == 0.5` would have failed the moment they pushed the change.
...
More invariants you can assert the same way:

```rust
// Commutativity: order of summation doesn't matter
proptest!(|(a: f64, b: f64)| {
    assert_eq!(a + b, b + a);
});

// Reverse-complement preserves GC content
proptest!(|(seq: Vec<u8>)| {
    let filtered: Vec<u8> = seq.iter().copied()
        .filter(|b| matches!(b, b'A' | b'C' | b'G' | b'T'))
        .collect();
    assert_eq!(gc_content(&filtered), gc_content(&reverse_complement(&filtered)));
});
...
Property tests find edge cases you'd never write by hand. They run hundreds of randomly-generated inputs per test.
...
A function **call** has overhead: pushing arguments, jumping to the function, returning. For tiny functions that run millions of times, that overhead matters.

**Inlining** copies the function's body directly into the caller — no jump, no return.
...
Spawning a thread isn't free. The OS pays a few microseconds per thread; rayon's pool amortises that — but the per-item work has to be larger than the per-item overhead.

![Three bars on the same time scale: thread-startup overhead is roughly the same size as one GC computation on a short read, but tiny compared to one GC on a long read.](images/lec1-context-switch-vs-work.svg){fig-alt="Horizontal bar chart with three rows. Row 1 (red): 'one OS context switch' at ~1-10 µs. Row 2 (orange): 'gc_content on a 1000-bp read' at ~1 µs. Row 3 (green): 'gc_content on a 10000-bp read' at ~10 µs. Annotation: if each item is faster than the overhead, parallelism makes things slower."}
...
- **Observed sublinear** — what you actually measure. The gap widens with core count: memory bandwidth is shared, caches contend, and there's a fixed serial fraction (Amdahl's law).
